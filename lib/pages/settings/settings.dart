...
class SettingsController extends State<Settings> {
  ...
  Future<void> changeLanguageAction() async {
    final selected = await showModalActionPopup<Locale>(
      context: context,
      title: 'Язык',
      actions: L10n.supportedLocales
          .map(
            (locale) => AdaptiveModalAction(
              label: locale.toLanguageTag(),
              value: locale,
              isDefaultAction:
                  locale.languageCode == AppSettings.appLocale.value,
            ),
          )
          .toList(),
    );
    if (selected == null) return;
    await AppSettings.appLocale.setItem(selected.languageCode);
    FluffyChatApp.localeNotifier.value = selected;
  }
