...
extension ClientDownloadContentExtension on Client {
  Future<Uint8List> downloadMxcCached(
    ...
    if (!mxc.isScheme('mxc') || mxc.host.isEmpty) {
      throw ArgumentError.value(mxc, 'mxc', 'Invalid MXC URI');
    }
    ...
    if ((httpUri.scheme != 'http' && httpUri.scheme != 'https') ||
        httpUri.host.isEmpty) {
      throw StateError(
        'Unable to resolve MXC URI "$mxc" for homeserver "$homeserver"',
      );
    }
